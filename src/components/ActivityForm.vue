...
function nextStep() {
  ...
  currentStep.value++
  // currentFormData = ref(activityForm.value.find((step) => step.id === currentStep.value))
}
</script>

<template>
  <form @submit.prevent="nextStep()">
    <h2 class="text-4xl font-bold">{{ activityFormData[currentStep].title }}</h2>
  ...
  <!-- <h2 class="text-4xl font-bold">{{ step.title }}</h2>
  <form class="py-8 text-xl" @submit.prevent="nextStep()">
